...
def feed_view(request):
    ...
    feed = (
        Ticket.objects.filter(Q(author=user) | Q(author__in=followed_users) | Q(review__user=user))
        .distinct()
        .annotate(combined_created_at=Coalesce("review__time_edited", F("created_at")))
...
def posts_view(request):
    user = request.user
    posts = (
        Ticket.objects.filter(Q(author=user) | Q(review__user=user))
        .prefetch_related(Prefetch("review_set", queryset=Review.objects.filter(user=user)))
        .annotate(combined_created_at=Coalesce("review__time_edited", F("created_at")))
        .order_by("-combined_created_at")
    )
